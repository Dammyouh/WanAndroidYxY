...
const val TAG = "MineVModel"

class MineVModel(application: Application) : AndroidViewModel(application) {
    ...
    fun registerUser() {
        ...
        call?.enqueue(object : Callback<BaseCallBackBean> {
            ...
            override fun onResponse(call: Call<BaseCallBackBean>?, response: Response<BaseCallBackBean>?) {
    ...
    fun loginUser(userName: String, pwd: String) {
        ...
        call?.enqueue(object : Callback<BaseCallBackBean> {
            ...
            override fun onResponse(call: Call<BaseCallBackBean>?, response: Response<BaseCallBackBean>?) {
                Log.d("yxy", "登录接口请求成功 call = $call ,response =  ${response?.body()} ")
                val body: BaseCallBackBean? = response?.body()
                val code = body?.errorCode
